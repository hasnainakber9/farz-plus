...
export function FamilyDocuments({ elderId }: { elderId: string | null }) {
  ...
  return (
    <section className="mt-4 overflow-hidden rounded-md border border-[#D5E4E0] bg-white">
      ...
      <div className="divide-y divide-[#E3ECE9]">{documents.map((document) => <div key={document.id} className="flex items-center justify-between gap-3 px-5 py-3"><div className="min-w-0"><p className="truncate text-sm font-bold text-[#143A35]">{document.name}</p><p className="mt-1 text-xs text-[#80948F]">{Math.max(1, Math.round(document.size_bytes / 1024))} KB · {document.consent_state === "VERIFIED" ? "Consent verified" : "Consent review pending"}</p></div><div className="flex items-center gap-1"><button type="button" onClick={() => openDocument(document.id)} className="grid h-9 w-9 place-items-center rounded-md text-[#087B69] hover:bg-[#EAF8F4]" title="Open document"><Download className="h-4 w-4" /></button><button type="button" onClick={() => removeDocument(document.id)} className="grid h-9 w-9 place-items-center rounded-md text-[#A63E33] hover:bg-[#FFF0EE]" title="Remove document"><X className="h-4 w-4" /></button></div></div>)}{!documents.length ? <p className="px-5 py-5 text-sm text-[#70847E]">No documents have been added.</p> : null}</div>
    </section>
  );
}
